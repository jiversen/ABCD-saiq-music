...
activities <- unlist(strapplyc(names(actData),"sports_activity_(.*)_school$"))
...
for (act0 in activities) {
  ...
  act <- paste0('sports_activity_',act0)
  ...
  for (event in c('baseline_year_1_arm_1','1_year_follow_up_y_arm_1','2_year_follow_up_y_arm_1')) {
    ...
    hist(adata[[paste0(act,'_ageStarted')]], main=paste0(act0,' ageStarted'),freq=FALSE)
    hist(adata[[paste0(act,"_hrperwk")]], main=paste0(act0,' Hrs/wk'),xlim=c(0, 8),freq=FALSE,breaks=seq(0,24*7,0.5))
...
```{r}
  ...
  adata[!is.na(adata$sports_activity_music_hrperwk) & adata$sports_activity_music_hrperwk==0, 'sports_activity_music_hrperwk']=NA
  
  ggplot(adata,aes(x=tpname,y=sports_activity_music_hrperwk))+geom_violin(draw_quantiles = c(0.5))+ylim(0,hrlim)+
     theme(text = element_text(size = 20)) 
    #scale_fill_viridis(discrete=TRUE) +
    #scale_color_viridis(discrete=TRUE) 

    ggplot(adata,aes(x=tpname,y=sports_activity_music_hrperwk))+geom_boxplot() 

  
  ggplot(adata,aes(x=sports_activity_music_hrperwk, fill=tpname)) + geom_histogram(position='identity',alpha=0.3,bins=50)
...
hrlim=1.5
  ...
  ggplot(adata,aes(x=household.income,y=sports_activity_music_hrperwk))+geom_violin(draw_quantiles = c(0.5))+ylim(0,hrlim)+
     theme(text = element_text(size = 20)) 
    #scale_fill_viridis(discrete=TRUE) +
    #scale_color_viridis(discrete=TRUE) 
  
    ggplot(adata,aes(x=high.educ,y=sports_activity_music_hrperwk))+geom_violin(draw_quantiles = c(0.5))+ylim(0,hrlim)+
     theme(text = element_text(size = 20)) 
  ...
  IV='household.income'
  IV='high.educ'
  DV='sports_activity_music_hrperwk'
