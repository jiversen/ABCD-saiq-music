...
# call after running merge_data_music, core_demographics_music and categorical_extension_music
# for now, analyze only baseline, not looking at change
...
#encoding is different **onyl after running categorical_extension code!
# Yes/No/NA
# Yes/No/not endorsed/NA -- for the weird ___0 & ___1 fields
# our code below expects boolean (1/0) coding
# 60 (1 hr)
...
for (act in activities) {
  ...
  #true for activity present in any context
  ...
  #add an approximate 'age started'
  adata[,paste0(act,'_ageStarted')] = adata$ageYrs - adata[,nyr]
...
corrplot(actCor, method="square",order="hclust",diag=FALSE,addrect=10,cl.lim=c(-lim, lim),p.mat = p.value, sig.level=0.05, insig="blank")
