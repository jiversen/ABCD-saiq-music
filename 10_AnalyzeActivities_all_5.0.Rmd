...
Analyze ABCD Activities questionnaire from ABCD 5.0 (doi:10.15154/8873-zj65 ). 
...
# Load Data from 01_preprocessActivities_5.0.R, if needed
```{r load-data}
# have to define the timestamp of data we wish to use
ts <- "2023-11-29_15-40-57"
...
if (!"actData" %in% ls()) {
  activitiesFile      <- file.path(resultDir,"abcd_5.0_activities.Rds")
  ...
  activitiesFileMusic <- file.path(resultDir, "abcd_5.0_activities_music.csv")
  activitiesFileInst <- file.path(resultDir, "abcd_5.0_activities_inst.Rds")
...
In 5.0 We have full activities data for baseline, year 1, year 2 and year 3; partial for year 4.

# Plot basic facts about each activity by timepoint

```{r activity-description-by-timepoint, fig.width=10, fig.height=6}
...
adata <- actData[, c('src_subject_id','eventname','ageYrs', measVar, endorsedVar)]
...
adataw <- adata %>% select(-eventname) %>% pivot_wider(names_from = tpname, names_sep='.', values_from = all_of(c("ageYrs", measVar, endorsedVar)))
...
#% by region
regional <- actData %>% group_by(site_id_l) %>% dplyr::mutate(siteN = n(), musicPerc = 100 * sum(music_p12,na.rm=TRUE)/n(), musicN = sum(music_p12,na.rm=TRUE))
...
#5.0 range from 52% to 19% by site
...
#demoVars <- c("src_subject_id","eventname","site_id_l","rel_family_id","rel_group_id","age", "sex","household.income","high.educ","anthro_bmi_calc","race_ethnicity",'EUR', 'AFR', 'EAS', 'AMR',"accult_phenx_q1_p","accult_phenx_q1")
...
ad = actData[,c(IVs,DVs,covariates,"site_id_l","rel_family_id")]
...
model_table_gamm = run_models(model_table, ad, model_type='GAMM4', cores=1,QN_IVs=FALSE,QN_gauss_DVs=FALSE)
...
# display fit results
```{r model-results, eval=FALSE, echo=FALSE, , fig.width=10, fig.height=10}
